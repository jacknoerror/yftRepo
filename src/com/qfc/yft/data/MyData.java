...
import com.qfc.yft.ui.tabs.chat.ConversationListAdapter;
...
//		return peopleMap.get(pid);
	...
	public ConversationListAdapter getConvListAdapter() {
		if(null==convListAdapter){
			convListAdapter = new ConversationListAdapter(MyApplication.app());
			//2.AllAdapterControl
			AllAdapterControl.getInstance().setConversationlistAdapter(convListAdapter);
			//3.buildData
			final List<Long> idArrs = BuildData.getInstance().fullConversation(MyApplication.app(), convListAdapter);
			new Thread() {
				public void run() {
