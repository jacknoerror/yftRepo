...
public class LIICompany extends MspJsonItem {
	...
	public void initJackJson(JSONObject job) throws JSONException {
		...
		//由于服务器返回字段不统一 由此代码
		if(job.has("compId")) shopId = job.optInt("compId");
		if(job.has("compName")) shopName = job.optString("compName");
		if(job.has("shopType")) shopType = job.optInt("shopType");
		if(job.has("shopLevel")) shopType = job.optInt("shopLevel");
